...
static bool parseHttpsUrl(const char *url, ParsedUrl &out) {
  if (!url) {
    return false;
  }
  ...
  if (!full.startsWith(httpsPrefix)) {
    return false;
  }

  String rest = full.substring(httpsPrefix.length());
...
static bool writeAll(WiFiClientSecure &client, const uint8_t *data, size_t length) {
  size_t sent = 0;
  while (sent < length) {
...
static esp_err_t trigger_capture_handler(httpd_req_t *req) {
  ...
  ParsedUrl backend;
  if (!parseHttpsUrl(BACKEND_PREDICT_URL, backend)) {
    esp_camera_fb_return(fb);
  ...
  if (!secureClient.connect(backend.host.c_str(), backend.port)) {
    esp_camera_fb_return(fb);
  ...
  const size_t contentLength = partHeader.length() + fb->len + partFooter.length();
  ...
  bool writeOk = true;
  writeOk = writeOk && writeAll(secureClient, reinterpret_cast<const uint8_t *>(requestHeaders.c_str()), requestHeaders.length());
  writeOk = writeOk && writeAll(secureClient, reinterpret_cast<const uint8_t *>(partHeader.c_str()), partHeader.length());
  writeOk = writeOk && writeAll(secureClient, fb->buf, fb->len);
  writeOk = writeOk && writeAll(secureClient, reinterpret_cast<const uint8_t *>(partFooter.c_str()), partFooter.length());
  ...
  while (secureClient.connected() || secureClient.available()) {
    while (secureClient.available()) {
      char c = static_cast<char>(secureClient.read());
      rawResponse += c;
      startMs = millis();
    }
